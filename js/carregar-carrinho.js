...
document.addEventListener("DOMContentLoaded", () => {
  onAuthStateChanged(auth, (user) => {
    ...
    if (user) {
          ...
          if (modalPerfil) {
            ...
            if (btnAdmin) {
              btnAdmin.style.display =
                user.email === "admin@admin.com" ? "flex" : "none";
            }
      ...
      const listaProdutosEl = document.getElementById(
...
window.calcularFretePorCep = function () {
  ...
  if (inputCep.length !== 8) {
    if (erroCep) {
      ...
      erroCep.textContent =
        "Digite um CEP válido com 8 dígitos para Fortaleza e região.";
    }
    return;
  }
  ...
  if (erroCep) erroCep.style.display = "none";

  atualizarResumoValores();
  Swal.fire(
    "Frete Calculado",
    "Taxa de entrega adicionada: R$ 15,00",
    "success",
  );
};
...
async function executarEnvioOrdemServico(nome, telefone, cpf, endereco) {
  ...
  try {
    ...
    Swal.fire(
      ...
      "Não foi possível registrar sua venda. Por favor, tente novamente.",
      "error",
    );
  ...
  try {
    ...
    const textoOrdemServico = `ORDEM DE SERVIÇO
---------------------------
DADOS CLIENTE 
NOME: ${nome}
...
TOTAL: R$ ${valorFinal.toFixed(2)}`;
    ...
    if (!data.url) {
      throw new Error(
        "A resposta do servidor não incluiu uma URL de checkout válida.",
      );
    }
